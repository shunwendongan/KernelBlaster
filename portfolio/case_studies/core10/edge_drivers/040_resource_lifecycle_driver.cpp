...
bool buffers_are_independent(const Workload& first, const Workload& second) {
    return first.input.data_ptr() != second.input.data_ptr()
        && first.weight.data_ptr() != second.weight.data_ptr()
        && first.bias.data_ptr() != second.bias.data_ptr()
...
Workload make_workload(int64_t seed, int64_t salt) {
    ...
    const std::vector<int64_t> normalized = {kFeatures, kDim1, kDim2};
    auto input = torch::randn(
        {kBatch, kFeatures, kDim1, kDim2}, options
    );
...
bool run_calls(
    ...
    for (int call = 0; call < calls; ++call) {
        ...
        launch_gpu_implementation(
            ...
            workload.input.data_ptr(),
            workload.weight.data_ptr(),
            workload.bias.data_ptr(),
            kBatch,
            kFeatures,
            kDim1,
            kDim2
...
LeakAudit audit_thread_exit_release(Workload& first, Workload& second) {
    ...
    if (!audit.bounded) {
        std::cerr
            << "KERNELBLASTER_LIFECYCLE_LEAK task=040 steady_state_decrease="
