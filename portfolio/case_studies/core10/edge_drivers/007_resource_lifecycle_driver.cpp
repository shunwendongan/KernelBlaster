...
constexpr int kMeasuredThreadWaves = 8;
...
bool buffers_are_independent(const Workload& first, const Workload& second) {
    return first.input_a.data_ptr() != second.input_a.data_ptr()
        && first.input_b.data_ptr() != second.input_b.data_ptr()
...
Workload make_workload(int64_t seed, int64_t salt) {
    ...
    auto input_a = torch::randn({kM, kK}, options);
    auto input_b = torch::randn({kK, kN}, options);
    auto reference = torch::matmul(
        input_a.to(torch::kFloat32), input_b.to(torch::kFloat32)
    );
    return {
        input_a,
        input_b,
        reference,
        torch::empty({kM, kN}, options),
    };
}
...
bool run_calls(
    ...
    for (int call = 0; call < calls; ++call) {
        ...
        launch_gpu_implementation(
            ...
            workload.input_a.data_ptr(),
            workload.input_b.data_ptr(),
            kM,
            kN,
            kK
        );
...
LeakAudit audit_thread_exit_release(Workload& first, Workload& second) {
    ...
    if (!audit.bounded) {
        std::cerr
            << "KERNELBLASTER_LIFECYCLE_LEAK task=007 steady_state_decrease="
