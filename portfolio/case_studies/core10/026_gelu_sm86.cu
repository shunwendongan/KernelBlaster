...
__device__ __forceinline__ float gelu(float value) {
    ...
    return 0.5f * value * (1.0f + tanhf(argument));
}

__global__ void gelu_pack8_kernel(
...
#pragma unroll
    for (int pair = 0; pair < 4; ++pair) {
        ...
        data.values[pair] = __floats2half2_rn(gelu(value.x), gelu(value.y));
    }
    reinterpret_cast<uint4*>(output)[pack] = data.bits;
}

__global__ void gelu_tail_kernel(
    half* output,
    const half* input,
    int64_t begin,
    int64_t total
    ...
    if (index < total) {
        output[index] = __float2half_rn(gelu(__half2float(input[index])));
    }
}
...
void launch_gpu_implementation(
    ...
    if (packs) {
        gelu_pack8_kernel<<<static_cast<int>((packs + threads - 1) / threads), threads>>>(
