...
static bool run_case(
    ...
    auto b = torch::randn({shape.k, 1}, options);
    auto reference = torch::matmul(a.to(torch::kFloat32), b.to(torch::kFloat32));
    auto output = torch::empty({shape.m, 1}, options);
    torch::Tensor first;
    bool deterministic = true;
    for (int repeat = 0; repeat < 5; ++repeat) {
        launch_gpu_implementation(
            output.data_ptr(), a.data_ptr(), b.data_ptr(), shape.m, shape.k
        );
...
int main() {
    const std::vector<Shape> shapes = {
        {"canonical", 256, 131072},
        {"boundary-1x1", 1, 1},
        {"odd-17x31", 17, 31},
        {"odd-33x65", 33, 65},
        {"neighbor-257x129", 257, 129},
    };
